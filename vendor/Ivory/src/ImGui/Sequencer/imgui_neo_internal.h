...
namespace ImGui {
    ...
    IMGUI_API void  RenderNeoSequencerTopBarOverlay(float time_per_frame, float zoom, float valuesWidth, uint32_t startFrame, uint32_t endFrame, uint32_t offsetFrame, const ImVec2& cursor, const ImVec2& size, ImDrawList* drawList = nullptr, bool drawFrameLines = true, bool drawFrameText = true, float maxPixelsPerTick = -1.0f);
