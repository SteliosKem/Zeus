...
namespace ImGui {
    ...
    void
        RenderNeoSequencerTopBarOverlay(float time_per_frame, float zoom, float valuesWidth, uint32_t startFrame, uint32_t endFrame, uint32_t offsetFrame, const ImVec2& cursor, const ImVec2& size,
        ...
        if (drawFrameLines) {
            ...
            for (int32_t i = 0; i < count; i++) {
                ...
                if (drawFrameText && secondaryFrame) {
                    char text[6];
                    const auto printRes = snprintf(text, sizeof(text), "%g", (viewStart + i) * time_per_frame);
                    if (printRes > 0) {
